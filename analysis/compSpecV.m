...
for i = 1:1:numTraj
    ...
    for j = 1:1:(numInts/Trec)
        ...
        Vact_fit_pitU(:,j) = polyfit(log10(f_nth_dec(Vk_inds_pit{j})),log10(Vppsdw_avg((Vk_inds_pit{j}),j).*((f_nth_dec(Vk_inds_pit{j}))'.^(-5/3))),1);
...
for pt = 1:1:(numInts/Trec)
    ...
    tmpptV = reshape(VppdfPlt(:,pt,:),[length(freq),numTraj]);
    avVppsd(:,pt) = mean(tmpptV,2);
