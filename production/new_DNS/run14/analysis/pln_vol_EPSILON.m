...
epsDNSzSD = std2(10.^(epsDNSz));
log_Wepsilon_kSD = std2(10.^(log_Wepsilon_k));
mnepsDNSzSD = std2(10.^(mnepsDNSz));
...
for j = 1:1:(numInts/Trec)
    for k=1:length(pltdiffF)
        unConstSlpUTrajplt(j,k,:) = medfilt1(unConstSlpUTraj(j,k,:),5,'omitnan','truncate');
        unConstSlpVTrajplt(j,k,:) = medfilt1(unConstSlpVTraj(j,k,:),5,'omitnan','truncate');
        unConstSlpWTrajplt(j,k,:) = medfilt1(unConstSlpWTraj(j,k,:),5,'omitnan','truncate');
    end
end
%}
slpArr = [-10 3*(-5/3) 2*(-5/3) 1.5*(-5/3) 1.4*(-5/3) 1.3*(-5/3) 1.2*(-5/3) 1.1*(-5/3) (-5/3) 0.9*(-5/3) 0.8*(-5/3) ...
    0.7*(-5/3) 0.6*(-5/3) 0.5*(-5/3) 0.1*(-5/3)];
cmap = turbo(length(slpArr));
...
for j = 1:1:(numInts/Trec)
    ...
    caxis([4*(-5/3) 0])
    col1 = colorbar('Ticks',slpArr,...
             'TickLabels',{'4*(-5/3)' '3*(-5/3)' '2*(-5/3)' '1.5*(-5/3)' '1.2*(-5/3)' '(-5/3)' '0.5*(-5/3)' '0.1*(-5/3)'},'Location','southoutside');
    ...
    tl2 = nexttile;
    contourf(log10(Fx),Hz,reshape(unConstSlpUTrajplt(j,:,:),[length(pltdiffF) length(smplN0)])',[-5/3-0.5 -5/3+0.5])
    %colormap('hot')
    colormap(cmap)
    xlim([min(log10(pltdiffF)) max(log10(pltdiffF))])
    ylim([min(smplN0) max(smplN0)])
    ...
    caxis([4*(-5/3) 0])
    col1 = colorbar('Ticks',slpArr,...
             'TickLabels',{'0.7*(-5/3)' '0.8*(-5/3)' '0.9*(-5/3)' '(-5/3)' '1.1*(-5/3)' '1.2*(-5/3)' '1.3*(-5/3)'},'Location','southoutside');
    col1.Label.String = 'fit slope';
    %colormap('hot')
    colormap(cmap)
    xlim([min(log10(pltdiffF)) max(log10(pltdiffF))])
    ...
    ylabel('sample number')
    tl2 = nexttile;
    contourf(log10(Fx),Hz,reshape(unConstSlpVTrajplt(j,:,:),[length(pltdiffF) length(smplN0)])',[-5/3-0.5 -5/3+0.5])
    %colormap('hot')
    colormap(cmap)
    xlim([min(log10(pltdiffF)) max(log10(pltdiffF))])
    ylim([min(smplN0) max(smplN0)])
    ...
    ylabel('sample number')
    tl2 = nexttile;
    contourf(log10(Fx),Hz,reshape(unConstSlpWTrajplt(j,:,:),[length(pltdiffF) length(smplN0)])',[-5/3-0.5 -5/3+0.5])
    %colormap('hot')
    colormap(cmap)
    xlim([min(log10(pltdiffF)) max(log10(pltdiffF))])
    ylim([min(smplN0) max(smplN0)])
