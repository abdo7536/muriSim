...
for i = 1:1:numTraj
    ...
    for j = 1:1:(numInts/Trec)
        ...
        for q = 1:length(f_nth_dec)-1
            ...
            unConstSlpU(j,q) = tmp1(:,1);
            unConstLvlU(j,q) = tmp1(:,2);
        end
        ...
        Upit_NFw = log10(pit_NF*freq(f_ind_low:f_ind_high).^(5/3));
        Upit_NFw_avg = log10(pit_NF*f_nth_dec.^(5/3));
        ...
        Ulog_pwp_k(j) = mean((Upwp_est(Uk_inds,j)));      % average over log of kept indexes
        Ulog_pwp_ks(j) = std((Upwp_est(Uk_inds,j)));      % standard deviation of the log mean
        Ulog_pwp_k_avg(j) = mean((Upwp_est_avg(Uk_inds_avg,j))); % average over log of kept indexes
        Ulog_pwp_ks_avg(j) = std((Upwp_est_avg(Uk_inds_avg,j))); % standard deviation of the log mean
        ...
        Ulog_ppsd_avg(:,j) = Uppsdw_avg(:,j)' + log10(f_nth_dec.^(-5/3));
        
        % second pass: include indexes where the fit function is above the noise floor
        Uk_inds_avg = find(Ulog_ppsd_est(:,j) > (Upit_NFw_avg+log10(f_nth_dec.^(-5/3)))');   % indexes of data points to keep
        ...
        if pass_3 == 1
            ...
            if length(Uk_inds_avg) > min_fit_pts
                ...
                for p = 1:1:length(Uk_inds_avg)-(min_fit_pts)+1
                   Ustand(p) = std((Upwp_est_avg(Uind_to_use,j)));
        ...
        Ulog_pwp_k_avg(j) = mean((Upwp_est_avg(Uk_inds_avg,j))); % average over log of kept indexes - in second pass
        Ulog_pwp_ks_avg(j) = std((Upwp_est_avg(Uk_inds_avg,j))); % standard deviation of the log mean - in second pass
...
for pt = 1:1:(numInts/Trec)
    ...
    if pass_3 == 1
        ...
        if length(avUk_inds_avg) > min_fit_pts
            ...
            for p = 1:1:length(avUk_inds_avg)-(min_fit_pts)+1
                avUstand(p) = std(log10((avUpwp_est_avg(avUind_to_use,pt))));
    ...
    % poits needed (set by the user) to get a decent fit estimate, then
    % don't bother with such spectra [set to nan by default - generally indicates that all the points in the spectra are suspect]
    avUlog_pwp_k_avg(pt) = mean(log10((avUpwp_est_avg(avUk_inds_avg,pt)))); % average over log of kept indexes - in second pass
    avUlog_pwp_ks_avg(pt) = std(log10((avUpwp_est_avg(avUk_inds_avg,pt)))); % standard deviation of the log mean - in second pass
